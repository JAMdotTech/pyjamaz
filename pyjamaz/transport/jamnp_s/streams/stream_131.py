import logging

from jamcodec.base import JamBytes

from pyjamaz.transport.jamnp_s.message_types import MsgCE131SafroleTicketDistribution
from pyjamaz.transport.jamnp_s.stream_base import Stream, StreamType, StreamDirection

logger = logging.getLogger("pyjamaz.transport.jamnp_s")


class StreamSafroleTicketDistributionStep1(Stream):

    def __init__(self, stream_id: int, connection, direction: StreamDirection):
        super().__init__(stream_id, connection, direction)
        self.stream_type = StreamType.CE131_SafroleTicketDistributionStep1.value
        self.stream_type_byte = self.stream_type.to_bytes(length=1, byteorder='little')


    def initiator_reset(self, reset_code: int):
        logger.debug(f"CE131 received reset code: {reset_code}")
        self.protocol.ce131_ticket_distribution_failure(reset_code)
        super().initiator_reset(reset_code)


    def initiator_message(self, data: bytes):
        if len(data) == 0:
            logger.debug(f"CE131 initiator received empty data")
            return

        logger.warning(f"Unexpected data in CE131 initiator: {len(data)} bytes")
        self.handle_error("Unexpected data", 1)


    def acceptor_message(self, data: bytes):
        logger.debug(f"CE131 acceptor stream {self.stream_id} received ticket")
        msg = MsgCE131SafroleTicketDistribution.from_jam_bytes(JamBytes(data))
        self.protocol.ce131_received_ticket(self, msg)

    def peer_fin_received(self):
        super().peer_fin_received()
        self.protocol.ce131_ticket_distribution_success(0)