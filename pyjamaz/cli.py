...
def _parse_telemetry_endpoint(endpoint: str) -> Tuple[str, int]:
...
async def run(seed, port, ts, culprit, block_dir, record_traces, custom_db_path, verbose, host, bootnode, rpc_listen_ip, rpc_port, fuzzer, fuzzer_socket_path, telemetry_endpoint):
    ...
    telemetry_target: Tuple[str, int] = _parse_telemetry_endpoint(telemetry_endpoint))
    if telemetry_target:
