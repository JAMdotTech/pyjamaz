    ...
    while status == EXIT_RESUME and gas > 0:
        ...
        if inst_type == inst_none:
            if opcode == op_trap:
                ...
                if logging: log(logging, inst_nr, opcode, pc, reg, gas, context="error: unknown opcode", mem=section_arrays)
                return sync_state_and_return(reg, registers_out, EXIT_PANIC, status_out,
                                           pc, pc_out, gas, gas_out, inst_nr, inst_nr_out,
                                           exit_value, exit_value_out, ERROR_PANIC_TRAP)
        ...
        #GP-0.6.7-section:A.5.3
            ...
            if opcode == op_load_imm_64:
                reg[r_a] = v_x
                if logging: log(logging, inst_nr, opcode, pc, reg, gas, reg1=r_a, imm1=v_x, mem=section_arrays)
            else:
                if logging: log(logging, inst_nr, opcode, pc, reg, gas, context="error: unknown opcode", mem=section_arrays)
                return sync_state_and_return(reg, registers_out, EXIT_PANIC, status_out,
                                           pc, pc_out, gas, gas_out, inst_nr, inst_nr_out,
        ...
        #GP-0.6.7-section:A.5.4
            ...
            if opcode == op_store_imm_u8:
                ...
                if logging:
                    __s1, __v1 = mem_read_jit(v_x, U8(1), mem_section_starts, mem_section_ends, section_arrays, acl_dict)
                    log(logging, inst_nr, opcode, pc, reg, gas, imm1=v_x, imm2=v_y, context="u'_vx: " + str(__v1), mem=section_arrays)
                ...
                if mem_write_jit(v_x, v_y % (2**16), U8(2), mem_section_starts, mem_section_ends, section_arrays, acl_dict) < 0:
                    return sync_state_and_return(reg, registers_out, EXIT_PAGE_FAULT, status_out,
                                               ...
                                               exit_value, exit_value_out, ERROR_MEMORY_FAULT)
                if logging:
                    __s2, __v2 = mem_read_jit(v_x, U8(2), mem_section_starts, mem_section_ends, section_arrays, acl_dict)
                    log(logging, inst_nr, opcode, pc, reg, gas, imm1=v_x, imm2=v_y, context="u'_vx: " + str(__v2), mem=section_arrays)
                ...
                if mem_write_jit(v_x, v_y % (2**32), U8(4), mem_section_starts, mem_section_ends, section_arrays, acl_dict) < 0:
                    return sync_state_and_return(reg, registers_out, EXIT_PAGE_FAULT, status_out,
                                               ...
                                               exit_value, exit_value_out, ERROR_MEMORY_FAULT)
                if logging:
                    __s4, __v4 = mem_read_jit(v_x, U8(4), mem_section_starts, mem_section_ends, section_arrays, acl_dict)
                    log(logging, inst_nr, opcode, pc, reg, gas, imm1=v_x, imm2=v_y, context="u'_vx: " + str(__v4), mem=section_arrays)
                ...
                if mem_write_jit(v_x, v_y, U8(8), mem_section_starts, mem_section_ends, section_arrays, acl_dict) < 0:
                    return sync_state_and_return(reg, registers_out, EXIT_PAGE_FAULT, status_out,
                                               ...
                                               exit_value, exit_value_out, ERROR_MEMORY_FAULT)
                if logging:
                    __s8, __v8 = mem_read_jit(v_x, U8(8), mem_section_starts, mem_section_ends, section_arrays, acl_dict)
                    log(logging, inst_nr, opcode, pc, reg, gas, imm1=v_x, imm2=v_y, context="u'_vx: " + str(__v8), mem=section_arrays)
            else:
                if logging: log(logging, inst_nr, opcode, pc, reg, gas, context="error: unknown opcode", mem=section_arrays)
                return sync_state_and_return(reg, registers_out, EXIT_PANIC, status_out,
                                           pc, pc_out, gas, gas_out, inst_nr, inst_nr_out,
        ...
        #GP-0.6.7-section:A.5.5
            ...
            if opcode == op_jump:
                skip_len = v_x
                if logging: log(logging, inst_nr, opcode, pc, reg, gas, off1=v_x, context="skip_len: " + str(v_x), mem=section_arrays)
            else:
                if logging: log(logging, inst_nr, opcode, pc, reg, gas, context="error: unknown opcode", mem=section_arrays)
                return sync_state_and_return(reg, registers_out, EXIT_PANIC, status_out,
                                           pc, pc_out, gas, gas_out, inst_nr, inst_nr_out,
        ...
        #GP-0.6.7-section:A.5.6
            ...
            if opcode == op_jump_ind:
                ...
                if djump_result == I32(-1):
                    ...
                    if logging: log(logging, inst_nr, opcode, pc, reg, gas, reg1=r_a, imm1=v_x, context="skip_len: " + str(djump_result), mem=section_arrays)
                ...
                status_read, loaded_value = mem_read_jit(v_x, U8(1), mem_section_starts, mem_section_ends, section_arrays, acl_dict)
                ...
                status_read, loaded_value = mem_read_jit(v_x, U8(2), mem_section_starts, mem_section_ends, section_arrays, acl_dict)
                if status_read != I32(0):
                    return sync_state_and_return(reg, registers_out, EXIT_PAGE_FAULT, status_out,
                                               pc, pc_out, gas, gas_out, inst_nr, inst_nr_out,
                                               exit_value, exit_value_out, ERROR_MEMORY_FAULT)
                ...
                status_read, loaded_value = mem_read_jit(v_x, U8(4), mem_section_starts, mem_section_ends, section_arrays, acl_dict)
                if status_read != I32(0):
                    return sync_state_and_return(reg, registers_out, EXIT_PAGE_FAULT, status_out,
                                               pc, pc_out, gas, gas_out, inst_nr, inst_nr_out,
                                               exit_value, exit_value_out, ERROR_MEMORY_FAULT)
                ...
                status_read, loaded_value = mem_read_jit(v_x, U8(8), mem_section_starts, mem_section_ends, section_arrays, acl_dict)
                if status_read != I32(0):
                    return sync_state_and_return(reg, registers_out, EXIT_PAGE_FAULT, status_out,
                                               pc, pc_out, gas, gas_out, inst_nr, inst_nr_out,
                                               exit_value, exit_value_out, ERROR_MEMORY_FAULT)
                ...
                if mem_write_jit(v_x, reg[r_a] % (2**8), U8(1), mem_section_starts, mem_section_ends, section_arrays, acl_dict) < 0:
                    return sync_state_and_return(reg, registers_out, EXIT_PAGE_FAULT, status_out,
                                               ...
                                               exit_value, exit_value_out, ERROR_MEMORY_FAULT)
                if logging:
                    _rs1, _rv1 = mem_read_jit(v_x, U8(1), mem_section_starts, mem_section_ends, section_arrays, acl_dict)
                    log(logging, inst_nr, opcode, pc, reg, gas, reg1=r_a, imm1=v_x, context="u'_vx: " + str(_rv1), mem=section_arrays)
                ...
                if mem_write_jit(v_x, reg[r_a] % (2**16), U8(2), mem_section_starts, mem_section_ends, section_arrays, acl_dict) < 0:
                    return sync_state_and_return(reg, registers_out, EXIT_PAGE_FAULT, status_out,
                                               ...
                                               exit_value, exit_value_out, ERROR_MEMORY_FAULT)
                if logging:
                    _rs2, _rv2 = mem_read_jit(v_x, U8(2), mem_section_starts, mem_section_ends, section_arrays, acl_dict)
                    log(logging, inst_nr, opcode, pc, reg, gas, reg1=r_a, imm1=v_x, context="u'_vx: " + str(_rv2), mem=section_arrays)
                ...
                if mem_write_jit(v_x, reg[r_a] % (2**32), U8(4), mem_section_starts, mem_section_ends, section_arrays, acl_dict) < 0:
                    return sync_state_and_return(reg, registers_out, EXIT_PAGE_FAULT, status_out,
                                               ...
                                               exit_value, exit_value_out, ERROR_MEMORY_FAULT)
                if logging:
                    _rs4, _rv4 = mem_read_jit(v_x, U8(4), mem_section_starts, mem_section_ends, section_arrays, acl_dict)
                    log(logging, inst_nr, opcode, pc, reg, gas, reg1=r_a, imm1=v_x, context="u'_vx: " + str(_rv4), mem=section_arrays)
                ...
                if mem_write_jit(v_x, reg[r_a], U8(8), mem_section_starts, mem_section_ends, section_arrays, acl_dict) < 0:
                    return sync_state_and_return(reg, registers_out, EXIT_PAGE_FAULT, status_out,
                                               ...
                                               exit_value, exit_value_out, ERROR_MEMORY_FAULT)
                if logging:
                    _rs8, _rv8 = mem_read_jit(v_x, U8(8), mem_section_starts, mem_section_ends, section_arrays, acl_dict)
                    log(logging, inst_nr, opcode, pc, reg, gas, reg1=r_a, imm1=v_x, context="u'_vx: " + str(_rv8), mem=section_arrays)
            ...
            if opcode == op_store_imm_ind_u8:
                ...
                if mem_write_jit(store_addr, v_y % (2**8), U8(1), mem_section_starts, mem_section_ends, section_arrays, acl_dict) < 0:
                    return sync_state_and_return(reg, registers_out, EXIT_PAGE_FAULT, status_out,
                                               pc, pc_out, gas, gas_out, inst_nr, inst_nr_out,
                                               exit_value, exit_value_out, ERROR_MEMORY_FAULT)
                if logging: log(logging, inst_nr, opcode, pc, reg, gas, reg1=r_a, imm1=v_x, imm2=v_y, context="u'_vx: " + str(mem_read_jit(store_addr, U8(1), mem_section_starts, mem_section_ends, section_arrays, acl_dict)), mem=section_arrays)
                ...
                if mem_write_jit(store_addr, v_y % (2**16), U8(2), mem_section_starts, mem_section_ends, section_arrays, acl_dict) < 0:
                    return sync_state_and_return(reg, registers_out, EXIT_PAGE_FAULT, status_out,
                                               pc, pc_out, gas, gas_out, inst_nr, inst_nr_out,
                                               exit_value, exit_value_out, ERROR_MEMORY_FAULT)
                if logging: log(logging, inst_nr, opcode, pc, reg, gas, reg1=r_a, imm1=v_x, imm2=v_y, context="u'_vx: " + str(mem_read_jit(store_addr, U8(2), mem_section_starts, mem_section_ends, section_arrays, acl_dict)), mem=section_arrays)
                ...
                if mem_write_jit(store_addr, v_y % (2**32), U8(4), mem_section_starts, mem_section_ends, section_arrays, acl_dict) < 0:
                    return sync_state_and_return(reg, registers_out, EXIT_PAGE_FAULT, status_out,
                                               pc, pc_out, gas, gas_out, inst_nr, inst_nr_out,
                                               exit_value, exit_value_out, ERROR_MEMORY_FAULT)
                if logging: log(logging, inst_nr, opcode, pc, reg, gas, reg1=r_a, imm1=v_x, imm2=v_y, context="u'_vx: " + str(mem_read_jit(store_addr, U8(4), mem_section_starts, mem_section_ends, section_arrays, acl_dict)), mem=section_arrays)
                ...
                if mem_write_jit(store_addr, v_y, U8(8), mem_section_starts, mem_section_ends, section_arrays, acl_dict) < 0:
                    return sync_state_and_return(reg, registers_out, EXIT_PAGE_FAULT, status_out,
                                               pc, pc_out, gas, gas_out, inst_nr, inst_nr_out,
                                               exit_value, exit_value_out, ERROR_MEMORY_FAULT)
                if logging: log(logging, inst_nr, opcode, pc, reg, gas, reg1=r_a, imm1=v_x, imm2=v_y, context="u'_vx: " + str(mem_read_jit(store_addr, U8(8), mem_section_starts, mem_section_ends, section_arrays, acl_dict)), mem=section_arrays)
        ...
        #GP-0.6.7-section:A.5.8
            ...
            if opcode == op_load_imm_jump:
                ...
                if logging: log(logging, inst_nr, opcode, pc, reg, gas, reg1=r_a, imm1=v_x, off1=v_y, mem=section_arrays)
        ...
        #GP-0.6.7-section:A.5.9
            ...
            if opcode == op_move_reg:
                ...
                for _ in range(64):
                    ...
                    val >>= 1
                reg[r_d] = count
                if logging: log(logging, inst_nr, opcode, pc, reg, gas, reg1=r_d, reg2=r_a, context="w'_d: " + str(reg[r_d]), mem=section_arrays)
            ...
            if opcode == op_store_ind_u8:
                ...
                if mem_write_jit(store_addr, w_a % (2**8), U8(1), mem_section_starts, mem_section_ends, section_arrays, acl_dict) < 0:
                    return sync_state_and_return(reg, registers_out, EXIT_PAGE_FAULT, status_out,
                                               pc, pc_out, gas, gas_out, inst_nr, inst_nr_out,
                                               exit_value, exit_value_out, ERROR_MEMORY_FAULT)
                if logging: log(logging, inst_nr, opcode, pc, reg, gas, reg1=r_a, reg2=r_b, imm1=v_x, context="w_a: "+ str(w_a % (2**8))+" w_b: " + str(w_b), mem=section_arrays)
                ...
                if mem_write_jit(store_addr, w_a % (2**16), U8(2), mem_section_starts, mem_section_ends, section_arrays, acl_dict) < 0:
                    return sync_state_and_return(reg, registers_out, EXIT_PAGE_FAULT, status_out,
                                               pc, pc_out, gas, gas_out, inst_nr, inst_nr_out,
                                               exit_value, exit_value_out, ERROR_MEMORY_FAULT)
                if logging: log(logging, inst_nr, opcode, pc, reg, gas, reg1=r_a, reg2=r_b, imm1=v_x, context="w_a: "+ str(w_a % (2**16))+" w_b: " + str(w_b), mem=section_arrays)
                ...
                if mem_write_jit(store_addr, w_a % (2**32), U8(4), mem_section_starts, mem_section_ends, section_arrays, acl_dict) < 0:
                    return sync_state_and_return(reg, registers_out, EXIT_PAGE_FAULT, status_out,
                                               pc, pc_out, gas, gas_out, inst_nr, inst_nr_out,
                                               exit_value, exit_value_out, ERROR_MEMORY_FAULT)
                if logging: log(logging, inst_nr, opcode, pc, reg, gas, reg1=r_a, reg2=r_b, imm1=v_x, context="w_a: "+ str(w_a % (2**32))+" w_b: " + str(w_b), mem=section_arrays)
                ...
                if mem_write_jit(store_addr, w_a, U8(8), mem_section_starts, mem_section_ends, section_arrays, acl_dict) < 0:
                    return sync_state_and_return(reg, registers_out, EXIT_PAGE_FAULT, status_out,
                                               pc, pc_out, gas, gas_out, inst_nr, inst_nr_out,
                                               exit_value, exit_value_out, ERROR_MEMORY_FAULT)
                if logging: log(logging, inst_nr, opcode, pc, reg, gas, reg1=r_a, reg2=r_b, imm1=v_x, context="w_a: "+ str(w_a) + " w_b: " + str(w_b), mem=section_arrays)

            elif opcode == op_load_ind_u8:
                load_addr = w_b + v_x
                status_read, loaded_value = mem_read_jit(load_addr, U8(1), mem_section_starts, mem_section_ends, section_arrays, acl_dict)
                if status_read != I32(0):
                    return sync_state_and_return(reg, registers_out, EXIT_PAGE_FAULT, status_out,
                                               pc, pc_out, gas, gas_out, inst_nr, inst_nr_out,
                                               exit_value, exit_value_out, ERROR_MEMORY_FAULT)
                ...
                load_addr = w_b + v_x
                status_read, loaded_value = mem_read_jit(load_addr, U8(2), mem_section_starts, mem_section_ends, section_arrays, acl_dict)
                if status_read != I32(0):
                    return sync_state_and_return(reg, registers_out, EXIT_PAGE_FAULT, status_out,
                                               pc, pc_out, gas, gas_out, inst_nr, inst_nr_out,
                                               exit_value, exit_value_out, ERROR_MEMORY_FAULT)
                ...
                load_addr = w_b + v_x
                status_read, loaded_value = mem_read_jit(load_addr, U8(4), mem_section_starts, mem_section_ends, section_arrays, acl_dict)
                if status_read != I32(0):
                    return sync_state_and_return(reg, registers_out, EXIT_PAGE_FAULT, status_out,
                                               pc, pc_out, gas, gas_out, inst_nr, inst_nr_out,
                                               exit_value, exit_value_out, ERROR_MEMORY_FAULT)
                ...
                load_addr = w_b + v_x
                status_read, loaded_value = mem_read_jit(load_addr, U8(8), mem_section_starts, mem_section_ends, section_arrays, acl_dict)
                if status_read != I32(0):
                    return sync_state_and_return(reg, registers_out, EXIT_PAGE_FAULT, status_out,
                                               pc, pc_out, gas, gas_out, inst_nr, inst_nr_out,
                                               exit_value, exit_value_out, ERROR_MEMORY_FAULT)
                ...
                if logging: log(logging, inst_nr, opcode, pc, reg, gas, reg1=r_a, reg2=r_b, imm1=v_x, context="w_b: " + str(w_b), mem=section_arrays)
                ...
                if logging: log(logging, inst_nr, opcode, pc, reg, gas, reg1=r_a, reg2=r_b, imm1=v_x, context="w'_a: " + str(reg[r_a]) + " w_b: " + str(w_b), mem=section_arrays)
        ...
        #GP-0.6.7-section:A.5.11
            ...
            if opcode == op_branch_eq:
                ...
                if branch_result == I32(-1):
                    ...
                    skip_len = v_x
                if logging: log(logging, inst_nr, opcode, pc, reg, gas, reg1=r_a, reg2=r_b, off1=v_x, context="skip_len: " + str(skip_len), mem=section_arrays)
        ...
        #GP-0.6.7-section:A.5.12
            ...
            if opcode == op_load_imm_jump_ind:
                ...
                if djump_result == I32(-1):
                    ...
                    skip_len = djump_result
                if logging: log(logging, inst_nr, opcode, pc, reg, gas, reg1=r_a, reg2=r_b, imm1=v_x, imm2=v_y, context="skip_len: " + str(skip_len), mem=section_arrays)
            else:
                if logging: log(logging, inst_nr, opcode, pc, reg, gas, context="error: unknown opcode", mem=section_arrays)
                return sync_state_and_return(reg, registers_out, EXIT_PANIC, status_out,
                                           pc, pc_out, gas, gas_out, inst_nr, inst_nr_out,
        ...
        #GP-0.6.7-section:A.5.13
            ...
            if opcode == op_add_32:
                ...
                if logging: log(logging, inst_nr, opcode, pc, reg, gas, reg1=r_d, reg2=r_a, reg3=r_d, context="w'_d: " + str(reg[r_d]), mem=section_arrays)
