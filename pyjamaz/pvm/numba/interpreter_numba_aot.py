...
def imul64wide_aot(a: I64, b: I64) -> (U64, U64):
...
@cc.export('smul_u64wide_aot', types.UniTuple(uint64, 2)(int64, uint64))
...
@cc.export('rori64_aot', uint64(uint64, uint64))
def rori64_aot(x: U64, shift_amount: U64) -> U64:
    """JIT-compiled rotate right for 64-bit integers."""
    return U64(((x >> shift_amount) | (x << (64 - shift_amount))) & 0xFFFFFFFFFFFFFFFF)


@cc.export('roli64_aot', uint64(uint64, uint64))
def roli64_aot(x: U64, shift_amount: U64) -> U64:
    """JIT-compiled rotate left for 64-bit integers."""
    return U64(((x << shift_amount) | (x >> (64 - shift_amount))) & 0xFFFFFFFFFFFFFFFF)


@cc.export('rori32_aot', uint32(uint32, uint32))
def rori32_aot(x: U32, shift_amount: U32) -> U32:
    """JIT-compiled rotate right for 32-bit integers."""
    return U32(((x >> shift_amount) | (x << (32 - shift_amount))) & 0xFFFFFFFF)


@cc.export('roli32_aot', uint32(uint32, uint32))
def roli32_aot(x: U32, shift_amount: U32) -> U32:
    """JIT-compiled rotate left for 32-bit integers."""
    return U32(((x << shift_amount) | (x >> (32 - shift_amount))) & 0xFFFFFFFF)


@cc.export('pvm_smod_aot', int64(int64, int64))
def pvm_smod_aot(a: I64, b: I64) -> I64:
    """
...
@cc.export('pvm_rtz_div_aot', int64(int64, int64))
def pvm_rtz_div_aot(a: I64, b: I64) -> I64:
    """
...
@cc.export('pvm_X_aot', uint64(uint64, uint64))
def pvm_X_aot(x: U64, n: U64) -> U64:
...
@cc.export('pvm_Z_aot', int64(uint64, uint64))
def pvm_Z_aot(a: U64, n: U64) -> I64:
...
@cc.export('count_leading_zeroes_aot', uint64(uint64, uint8))
def count_leading_zeroes_aot(value: U64, max_bits:U8) -> U64:
...
@cc.export('count_trailing_zeroes_aot', uint64(uint64, uint8))
def count_trailing_zeroes_aot(value: U64, max_bits: U8) -> U64:
...
@cc.export('reverse_bytes_aot', uint64(uint64))
def reverse_bytes_aot(x: U64) -> U64:
...
@cc.export('riscv_div_aot', int64(int64, int64))
def riscv_div_aot(a: I64, b: I64) -> I64:
...
@cc.export('pvm_Z_inv_aot', uint64(int64, uint8))
def pvm_Z_inv_aot(a: I64, n: U8) -> U64:
    """
...
@cc.export('read_uint_aot', uint64(uint8[::1], uint32, uint8))
def read_uint_aot(code: npt.NDArray[U8], addr: U32, length: U8) -> U64:
...
def mem_write_aot(addr: U64, value: U64, bytes_to_write: U8,
...
@cc.export('mem_read_aot', types.Tuple((int32, uint64))(
    uint64,        # addr
    uint8,         # bytes_to_read
    uint32[::1],   # section_starts
    uint32[::1],   # section_ends
...
def mem_read_aot(addr: U64, bytes_to_read: U8,
...
@cc.export('sync_state_and_return_aot', uint32(
    uint64[::1],  # reg
...
def hash_memory_segment_aot(section_array) -> U64:
    ...
    return _fmix64_aot(h)


@cc.export('get_memory_hash_aot', uint64(u8_array_list, int32))
def get_memory_hash_aot(section_arrays, seg_idx: I32):
    """Compute a 64-bit hash for the given memory segment (entire buffer)."""
    segment_hash = U64(0)
    if seg_idx >= 0 and seg_idx < len(section_arrays):
        segment_hash = hash_memory_segment_aot(section_arrays[seg_idx])
    return segment_hash


@cc.export('sbrk_aot', types.Tuple((uint64, int32))(
    uint64,        # size
...
def sbrk_aot(size: U64, current_heap_ptr: U64, next_section_start: U64,
    ...
    if new_heap_ptr > next_page_boundary:
        ...
        for page_nr in range(pages):
            key = U32((U32(next_page_nr) + U32(page_nr)) & U32_MASK)
            acl_dict[key] = I32(mem_writable)

    return new_heap_ptr, grew_flag


@cc.export('branch_aot', int32(uint32, int64, boolean, int32[::1]))
def branch_aot(pc: U32, offset: I64, condition: bool, pc_to_inst_index) -> I32:
...
@cc.export('djump_aot', int32(uint32, uint32[::1], uint32, int32[::1]))
def djump_aot(a: U32, jump_table, pc: U32, pc_to_inst_index) -> I32:
...
@cc.export('invoke_native_aot', int32(
    uint32,          # pc
    int64,           # gas
    uint32,          # inst_nr
    uint32,          # skip_len
    ...
    types.DictType(uint32, int32),  # acl_dict
    ...
    while status == EXIT_RESUME and gas > 0:
        ...
        if inst_type == inst_none:  # InstructionType.none
            if opcode == op_trap:
                ...
                return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PANIC,
                                                 pc, gas, inst_nr,
        ...
        # GP-0.6.7-section:A.5.2
            ...
            v_x = pvm_X_aot(read_uint_aot(code, pc + 1, l_x), l_x)
            ...
            if opcode == op_ecalli:
                ...
                return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_HOST_HALT,
                                                 pc, gas, inst_nr,
        ...
        # GP-0.6.7-section:A.5.4
            ...
            v_x = pvm_X_aot(read_uint_aot(code, pc + 2, l_x), np.uint8(l_x))
            v_y = pvm_X_aot(read_uint_aot(code, pc + 2 + l_x, l_y), np.uint8(l_y))

            if opcode == op_store_imm_u8:
                if mem_write_aot(v_x, U64(v_y) & U64(0xFF), U8(1), mem_section_starts, mem_section_ends, section_arrays, acl_dict) < 0:
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PAGE_FAULT,
                                                     pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_MEMORY_FAULT)
                if logg:
                    __s1, __v1 = mem_read_aot(v_x, U8(1), mem_section_starts, mem_section_ends, section_arrays,
                                              acl_dict)
                    log(logging, local_state, reg, imm1=v_x, imm2=v_y, context="u'_vx: " + str(__v1),
                        mem=section_arrays, mem_starts=mem_section_starts, mem_ends=mem_section_ends)
            elif opcode == op_store_imm_u16:
                if mem_write_aot(v_x, U64(v_y) & U64(0xFFFF), U8(2), mem_section_starts, mem_section_ends, section_arrays,
                                 acl_dict) < 0:
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PAGE_FAULT,
                                                     pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_MEMORY_FAULT)
                if logg:
                    __s2, __v2 = mem_read_aot(v_x, U8(2), mem_section_starts, mem_section_ends, section_arrays,
                                              acl_dict)
                    log(logging, local_state, reg, imm1=v_x, imm2=v_y, context="u'_vx: " + str(__v2),
                        mem=section_arrays, mem_starts=mem_section_starts, mem_ends=mem_section_ends)
            elif opcode == op_store_imm_u32:
                if mem_write_aot(v_x, U64(v_y) & U32_MASK, U8(4), mem_section_starts, mem_section_ends, section_arrays,
                                 acl_dict) < 0:
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PAGE_FAULT,
                                                     pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_MEMORY_FAULT)
                if logg:
                    __s4, __v4 = mem_read_aot(v_x, U8(4), mem_section_starts, mem_section_ends, section_arrays,
                                              acl_dict)
                    log(logging, local_state, reg, imm1=v_x, imm2=v_y, context="u'_vx: " + str(__v4),
                        mem=section_arrays, mem_starts=mem_section_starts, mem_ends=mem_section_ends)
            elif opcode == op_store_imm_u64:
                if mem_write_aot(v_x, v_y, U8(8), mem_section_starts, mem_section_ends, section_arrays, acl_dict) < 0:
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PAGE_FAULT,
                                                     pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_MEMORY_FAULT)
                if logg:
                    __s8, __v8 = mem_read_aot(v_x, U8(8), mem_section_starts, mem_section_ends, section_arrays,
                                              acl_dict)
        ...
        # GP-0.6.7-section:A.5.5
            ...
            l_x = min(4, inst_arg_len[inst_index])
            v_x = pvm_Z_aot(read_uint_aot(code, pc + 1, l_x), l_x)
        ...
        # GP-0.6.7-section:A.5.6
            ...
            if opcode == op_jump_ind:
                ...
                djump_result = djump_aot(jump_target, jump_table, pc, pc_to_inst_index)
                if djump_result == I32(-1):
                    skip_len = I64(0)
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_HALT,
                                                     pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_NONE)
                ...
                status_read, loaded_value = mem_read_aot(v_x, U8(1), mem_section_starts, mem_section_ends,
                ...
                status_read, loaded_value = mem_read_aot(v_x, U8(1), mem_section_starts, mem_section_ends,
                                                         section_arrays, acl_dict)
                if status_read != I32(0):
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PAGE_FAULT, pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_MEMORY_FAULT)
                reg[r_a] = pvm_X_aot(loaded_value, U8(1))
                if logg: log(logging, local_state, reg, reg1=r_a, imm1=v_x, mem=section_arrays, mem_starts=mem_section_starts, mem_ends=mem_section_ends)

            elif opcode == op_load_u16:
                status_read, loaded_value = mem_read_aot(v_x, U8(2), mem_section_starts, mem_section_ends,
                                                         section_arrays, acl_dict)
                if status_read != I32(0):
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PAGE_FAULT, pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_MEMORY_FAULT)
                ...
                if status_read != I32(0):
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PAGE_FAULT, pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_MEMORY_FAULT)
                reg[r_a] = pvm_X_aot(loaded_value, U8(2))
                if logg: log(logging, local_state, reg, reg1=r_a, imm1=v_x, mem=section_arrays, mem_starts=mem_section_starts, mem_ends=mem_section_ends)

            elif opcode == op_load_u32:
                status_read, loaded_value = mem_read_aot(v_x, U8(4), mem_section_starts, mem_section_ends,
                                                         section_arrays, acl_dict)
                if status_read != I32(0):
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PAGE_FAULT, pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_MEMORY_FAULT)
                ...
                if status_read != I32(0):
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PAGE_FAULT, pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_MEMORY_FAULT)
                reg[r_a] = pvm_X_aot(loaded_value, U8(4))
                if logg: log(logging, local_state, reg, reg1=r_a, imm1=v_x, mem=section_arrays, mem_starts=mem_section_starts, mem_ends=mem_section_ends)

            elif opcode == op_load_u64:
                status_read, loaded_value = mem_read_aot(v_x, U8(8), mem_section_starts, mem_section_ends,
                                                         section_arrays, acl_dict)
                if status_read != I32(0):
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PAGE_FAULT, pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_MEMORY_FAULT)
                ...
                if mem_write_aot(v_x, U64(reg[r_a]) & U64(0xFF), U8(1), mem_section_starts, mem_section_ends, section_arrays,
                                 acl_dict) < 0:
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PAGE_FAULT, pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_MEMORY_FAULT)
                if logg:
                    _rs1, _rv1 = mem_read_aot(v_x, U8(1), mem_section_starts, mem_section_ends, section_arrays,
                                              acl_dict)
                    log(logging, local_state, reg, reg1=r_a, imm1=v_x, context="u'_vx: " + str(_rv1),
                        mem=section_arrays, mem_starts=mem_section_starts, mem_ends=mem_section_ends)

            elif opcode == op_store_u16:
                if mem_write_aot(v_x, U64(reg[r_a]) & U64(0xFFFF), U8(2), mem_section_starts, mem_section_ends, section_arrays,
                                 acl_dict) < 0:
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PAGE_FAULT, pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_MEMORY_FAULT)
                if logg:
                    _rs2, _rv2 = mem_read_aot(v_x, U8(2), mem_section_starts, mem_section_ends, section_arrays,
                                              acl_dict)
                    log(logging, local_state, reg, reg1=r_a, imm1=v_x, context="u'_vx: " + str(_rv2),
                        mem=section_arrays, mem_starts=mem_section_starts, mem_ends=mem_section_ends)

            elif opcode == op_store_u32:
                if mem_write_aot(v_x, U64(reg[r_a]) & U32_MASK, U8(4), mem_section_starts, mem_section_ends, section_arrays,
                                 acl_dict) < 0:
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PAGE_FAULT, pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_MEMORY_FAULT)
                if logg:
                    _rs4, _rv4 = mem_read_aot(v_x, U8(4), mem_section_starts, mem_section_ends, section_arrays,
                                              acl_dict)
                    log(logging, local_state, reg, reg1=r_a, imm1=v_x, context="u'_vx: " + str(_rv4),
                        mem=section_arrays, mem_starts=mem_section_starts, mem_ends=mem_section_ends)

            elif opcode == op_store_u64:
                if mem_write_aot(v_x, reg[r_a], U8(8), mem_section_starts, mem_section_ends, section_arrays,
                                 acl_dict) < 0:
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PAGE_FAULT, pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_MEMORY_FAULT)
                if logg:
                    _rs8, _rv8 = mem_read_aot(v_x, U8(8), mem_section_starts, mem_section_ends, section_arrays,
                                              acl_dict)
                ...
                if logg: log(logging, local_state, reg, context="error: unknown opcode",
                                mem=section_arrays, mem_starts=mem_section_starts, mem_ends=mem_section_ends)
                return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PANIC, pc, gas, inst_nr,
                                                 exit_value, skip_len, ERROR_PANIC_TRAP)
        ...
        # GP-0.6.7-section:A.5.7
            ...
            v_x = pvm_X_aot(read_uint_aot(code, pc + 2, l_x), U8(l_x))

            l_y = min(4, max(0, inst_arg_len[inst_index] - l_x - 1))
            v_y = pvm_X_aot(read_uint_aot(code, pc + 2 + l_x, l_y), U8(l_y))

            if opcode == op_store_imm_ind_u8:
                store_addr = (U64(w_a) + U64(v_x)) & U64_MASK
                if mem_write_aot(store_addr, U64(v_y) & U64(0xFF), U8(1), mem_section_starts, mem_section_ends,
                                 section_arrays, acl_dict) < 0:
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PAGE_FAULT, pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_MEMORY_FAULT)
                if logg: log(logging, local_state, reg, reg1=r_a, imm1=v_x, imm2=v_y,
                                context="u'_vx: " + str(
                                    mem_read_aot(store_addr, U8(1), mem_section_starts, mem_section_ends,
                                                 section_arrays, acl_dict)), mem=section_arrays, mem_starts=mem_section_starts, mem_ends=mem_section_ends)

            elif opcode == op_store_imm_ind_u16:
                store_addr = (U64(w_a) + U64(v_x)) & U64_MASK
                if mem_write_aot(store_addr, U64(v_y) & U64(0xFFFF), U8(2), mem_section_starts, mem_section_ends,
                                 section_arrays, acl_dict) < 0:
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PAGE_FAULT, pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_MEMORY_FAULT)
                if logg: log(logging, local_state, reg, reg1=r_a, imm1=v_x, imm2=v_y,
                                context="u'_vx: " + str(
                                    mem_read_aot(store_addr, U8(2), mem_section_starts, mem_section_ends,
                                                 section_arrays, acl_dict)), mem=section_arrays, mem_starts=mem_section_starts, mem_ends=mem_section_ends)

            elif opcode == op_store_imm_ind_u32:
                store_addr = (U64(w_a) + U64(v_x)) & U64_MASK
                if mem_write_aot(store_addr, U64(v_y) & U32_MASK, U8(4), mem_section_starts, mem_section_ends,
                                 section_arrays, acl_dict) < 0:
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PAGE_FAULT, pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_MEMORY_FAULT)
                if logg: log(logging, local_state, reg, reg1=r_a, imm1=v_x, imm2=v_y,
                                context="u'_vx: " + str(
                                    mem_read_aot(store_addr, U8(4), mem_section_starts, mem_section_ends,
                                                 section_arrays, acl_dict)), mem=section_arrays, mem_starts=mem_section_starts, mem_ends=mem_section_ends)

            elif opcode == op_store_imm_ind_u64:
                store_addr = (U64(w_a) + U64(v_x)) & U64_MASK
                if mem_write_aot(store_addr, v_y, U8(8), mem_section_starts, mem_section_ends, section_arrays,
                                 acl_dict) < 0:
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PAGE_FAULT, pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_MEMORY_FAULT)
                if logg: log(logging, local_state, reg, reg1=r_a, imm1=v_x, imm2=v_y,
                                context="u'_vx: " + str(
                                    mem_read_aot(store_addr, U8(8), mem_section_starts, mem_section_ends,
                                                 section_arrays, acl_dict)), mem=section_arrays, mem_starts=mem_section_starts, mem_ends=mem_section_ends)
        ...
        # GP-0.6.7-section:A.5.8
            ...
            l_y = min(4, max(0, inst_arg_len[inst_index] - l_x - 1))
            v_y = pvm_Z_aot(read_uint_aot(code, pc + 2 + l_x, l_y), U8(l_y))
            ...
            if opcode == op_load_imm_jump:
                ...
                branch_result = branch_aot(pc, v_y, w_a == v_x, pc_to_inst_index)
                ...
                branch_result = branch_aot(pc, v_y, pvm_Z_aot(w_a, 8) < pvm_Z_aot(v_x, 8), pc_to_inst_index)
                if branch_result == I32(-1):
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PANIC, pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_PANIC_INVALID_BRANCH)
                elif pvm_Z_aot(w_a, 8) < pvm_Z_aot(v_x, 8):
                    skip_len = v_y
                if logg: log(logging, local_state, reg, reg1=r_a, imm1=v_x, off1=v_y,
                                mem=section_arrays, mem_starts=mem_section_starts, mem_ends=mem_section_ends)
        ...
        # GP-0.6.7-section:A.5.9
            ...
            if opcode == op_move_reg:
                ...
                new_heap_ptr, grew = sbrk_aot(size, current_heap_ptr, next_section_start, acl_dict, mem_writable_value,
                                              section_arrays, mem_section_starts)
                ...
                if new_heap_ptr != U64(0):
                    ...
                    if grew == I32(1):
                        # Signal wrapper to sync grown heap back to numpy
                        heap_grew_out[0] = 1
                ...
                reg[r_d] = count_leading_zeroes_aot(reg[r_a], U8(64))
                if logg: log(logging, local_state, reg, reg1=r_d, reg2=r_a,
                                context="w'_d: " + str(reg[r_d]), mem=section_arrays, mem_starts=mem_section_starts, mem_ends=mem_section_ends)

            elif opcode == op_leading_zero_bits_32:
                reg[r_d] = count_leading_zeroes_aot(U64(reg[r_a]) & U32_MASK, U8(32))
                if logg: log(logging, local_state, reg, reg1=r_d, reg2=r_a,
                                context="w'_d: " + str(reg[r_d]), mem=section_arrays, mem_starts=mem_section_starts, mem_ends=mem_section_ends)

            elif opcode == op_trailing_zero_bits_64:
                reg[r_d] = count_trailing_zeroes_aot(reg[r_a], U8(64))
                if logg: log(logging, local_state, reg, reg1=r_d, reg2=r_a,
                                context="w'_d: " + str(reg[r_d]), mem=section_arrays, mem_starts=mem_section_starts, mem_ends=mem_section_ends)

            elif opcode == op_trailing_zero_bits_32:
                reg[r_d] = count_trailing_zeroes_aot(U64(reg[r_a]) & U32_MASK, U8(32))
                if logg: log(logging, local_state, reg, reg1=r_d, reg2=r_a,
                                context="w'_d: " + str(reg[r_d]), mem=section_arrays, mem_starts=mem_section_starts, mem_ends=mem_section_ends)

            elif opcode == op_sign_extend_8:
                # todo: !!!!!!!!!!!!!!!!!!!reg[r_d] = pvm_X_jit(reg[r_a], U8(1))
                reg[r_d] = pvm_Z_inv_aot(pvm_Z_aot(U64(reg[r_a]) & U64(0xFF), 1), U8(8))
                if logg: log(logging, local_state, reg, reg1=r_d, reg2=r_a,
                                context="w'_d: " + str(reg[r_d]), mem=section_arrays, mem_starts=mem_section_starts, mem_ends=mem_section_ends)

            elif opcode == op_sign_extend_16:
                # todo: !!!!!!!!!!!!!!!reg[r_d] = pvm_X_jit(reg[r_a], U8(2))
                reg[r_d] = pvm_Z_inv_aot(pvm_Z_aot(U64(reg[r_a]) & U64(0xFFFF), 2), U8(8))
                if logg: log(logging, local_state, reg, reg1=r_d, reg2=r_a,
                                context="w'_d: " + str(reg[r_d]), mem=section_arrays, mem_starts=mem_section_starts, mem_ends=mem_section_ends)
                ...
                reg[r_d] = reverse_bytes_aot(reg[r_a])
                if logg: log(logging, local_state, reg, reg1=r_d, reg2=r_a,
                                context="w'_d: " + str(reg[r_d]), mem=section_arrays, mem_starts=mem_section_starts, mem_ends=mem_section_ends)
        ...
        # GP-0.6.7-section:A.5.10
            ...
            if opcode == op_store_ind_u8:
                ...
                if mem_write_aot(store_addr, U64(w_a) & U64(0xFF), U8(1), mem_section_starts, mem_section_ends,
                                 section_arrays, acl_dict) < 0:
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PAGE_FAULT, pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_MEMORY_FAULT)
                if logg: log(logging, local_state, reg, reg1=r_a, reg2=r_b, imm1=v_x,
                                context="w_a: " + str(U64(w_a) & U64(0xFF)) + " w_b: " + str(w_b), mem=section_arrays, mem_starts=mem_section_starts, mem_ends=mem_section_ends)

            elif opcode == op_store_ind_u16:
                store_addr = (U64(w_b) + U64(v_x)) & U64_MASK
                if mem_write_aot(store_addr, U64(w_a) & U64(0xFFFF), U8(2), mem_section_starts, mem_section_ends, section_arrays, acl_dict) < 0:
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PAGE_FAULT, pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_MEMORY_FAULT)
                if logg: log(logging, local_state, reg, reg1=r_a, reg2=r_b, imm1=v_x,
                ...
                store_addr = (U64(w_b) + U64(v_x)) & U64_MASK
                if mem_write_aot(store_addr, U64(w_a) & U32_MASK, U8(4), mem_section_starts, mem_section_ends, section_arrays, acl_dict) < 0:
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PAGE_FAULT, pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_MEMORY_FAULT)
                if logg: log(logging, local_state, reg, reg1=r_a, reg2=r_b, imm1=v_x,
                ...
                store_addr = (U64(w_b) + U64(v_x)) & U64_MASK
                if mem_write_aot(store_addr, w_a, U8(8), mem_section_starts, mem_section_ends, section_arrays,
                                 acl_dict) < 0:
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PAGE_FAULT, pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_MEMORY_FAULT)
                if logg: log(logging, local_state, reg, reg1=r_a, reg2=r_b, imm1=v_x,
                ...
                status_read, loaded_value = mem_read_aot(load_addr, U8(1), mem_section_starts, mem_section_ends,
                                                         section_arrays, acl_dict)
                if status_read != I32(0):
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PAGE_FAULT, pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_MEMORY_FAULT)
                ...
                if status_read != I32(0):
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PAGE_FAULT, pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_MEMORY_FAULT)
                reg[r_a] = pvm_Z_inv_aot(pvm_Z_aot(loaded_value, 1), U8(8))
                if logg: log(logging, local_state, reg, reg1=r_a, reg2=r_b, imm1=v_x,
                                context="w_a: " + str(w_a) + " w_b: " + str(w_b), mem=section_arrays)

            elif opcode == op_load_ind_u16:
                load_addr = (U64(w_b) + U64(v_x)) & U64_MASK
                status_read, loaded_value = mem_read_aot(load_addr, U8(2), mem_section_starts, mem_section_ends,
                                                         section_arrays, acl_dict)
                if status_read != I32(0):
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PAGE_FAULT, pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_MEMORY_FAULT)
                ...
                if status_read != I32(0):
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PAGE_FAULT, pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_MEMORY_FAULT)
                reg[r_a] = pvm_Z_inv_aot(pvm_Z_aot(loaded_value, 2), U8(8))
                if logg: log(logging, local_state, reg, reg1=r_a, reg2=r_b, imm1=v_x,
                                context="w_a: " + str(w_a) + " w_b: " + str(w_b), mem=section_arrays)

            elif opcode == op_load_ind_u32:
                load_addr = (U64(w_b) + U64(v_x)) & U64_MASK
                status_read, loaded_value = mem_read_aot(load_addr, U8(4), mem_section_starts, mem_section_ends,
                                                         section_arrays, acl_dict)
                if status_read != I32(0):
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PAGE_FAULT, pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_MEMORY_FAULT)
                ...
                if status_read != I32(0):
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PAGE_FAULT, pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_MEMORY_FAULT)
                reg[r_a] = pvm_Z_inv_aot(pvm_Z_aot(loaded_value, 4), U8(8))
                if logg: log(logging, local_state, reg, reg1=r_a, reg2=r_b, imm1=v_x,
                                context="w_a: " + str(w_a) + " w_b: " + str(w_b), mem=section_arrays)

            elif opcode == op_load_ind_u64:
                load_addr = (U64(w_b) + U64(v_x)) & U64_MASK
                status_read, loaded_value = mem_read_aot(load_addr, U8(8), mem_section_starts, mem_section_ends,
                                                         section_arrays, acl_dict)
                if status_read != I32(0):
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PAGE_FAULT, pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_MEMORY_FAULT)
                ...
                reg[r_a] = pvm_X_aot(U32(wb_vx_32), np.uint8(4))
                if logg: log(logging, local_state, reg, reg1=r_a, reg2=r_b, imm1=v_x,
                ...
                reg[r_a] = pvm_X_aot(U32(prod32), np.uint8(4))
                if logg: log(logging, local_state, reg, reg1=r_a, reg2=r_b, imm1=v_x,
                                context="w_b: " + str(w_b), mem=section_arrays)
                ...
                reg[r_a] = U64(1) if pvm_Z_aot(w_b, 8) < pvm_Z_aot(v_x, 8) else U64(0)
                if logg: log(logging, local_state, reg, reg1=r_a, reg2=r_b, imm1=v_x,
                                context="w_b: " + str(w_b), mem=section_arrays)

            elif opcode == op_shlo_l_imm_32:
                sh = U64(v_x) & U64(31)
                reg[r_a] = pvm_X_aot(U32((U64(w_b) << sh) & U32_MASK), U8(4))
                if logg: log(logging, local_state, reg, reg1=r_a, reg2=r_b, imm1=v_x,
                                context="w_b: " + str(w_b), mem=section_arrays)

            elif opcode == op_shlo_r_imm_32:
                # TODO!!!!!!!!!!?
                reg[r_a] = pvm_X_aot(U32(w_b) >> U32(U32(v_x) & U32(31)), U8(4))
                if logg: log(logging, local_state, reg, reg1=r_a, reg2=r_b, imm1=v_x,
                                context="w_b: " + str(w_b), mem=section_arrays)

            elif opcode == op_shar_r_imm_32:
                reg[r_a] = pvm_Z_inv_aot(I32(pvm_Z_aot(U32(w_b), 4)) >> I64(U32(v_x) & U32(31)), U8(8))
                if logg: log(logging, local_state, reg, reg1=r_a, reg2=r_b, imm1=v_x,
                                context="w'_a: " + str(reg[r_a]) + " w_b: " + str(w_b), mem=section_arrays)

            elif opcode == op_neg_add_imm_32:
                diff32 = (U64(v_x) - U64(w_b)) & U32_MASK
                reg[r_a] = pvm_X_aot(U32(diff32), U8(4))
                if logg: log(logging, local_state, reg, reg1=r_a, reg2=r_b, imm1=v_x,
                                context="w'_a: " + str(reg[r_a]) + " w_b: " + str(w_b), mem=section_arrays)
                ...
                sh = U64(w_b) & U64(31)
                reg[r_a] = pvm_X_aot(U32((U64(v_x) << sh) & U32_MASK), U8(4))
                if logg: log(logging, local_state, reg, reg1=r_a, reg2=r_b, imm1=v_x,
                                context="w'_a: " + str(reg[r_a]) + " w_b: " + str(w_b), mem=section_arrays)

            elif opcode == op_shlo_r_imm_alt_32:
                reg[r_a] = pvm_X_aot(U32(v_x) >> U32(U32(w_b) & U32(31)), U8(4))
                if logg: log(logging, local_state, reg, reg1=r_a, reg2=r_b, imm1=v_x,
                                context="w'_a: " + str(reg[r_a]) + " w_b: " + str(w_b), mem=section_arrays)

            elif opcode == op_shar_r_imm_alt_32:
                reg[r_a] = pvm_Z_inv_aot(I32(pvm_Z_aot(U32(v_x), 4)) >> I64(U32(w_b) & U32(31)), U8(8))
                if logg: log(logging, local_state, reg, reg1=r_a, reg2=r_b, imm1=v_x,
                                context="w'_a: " + str(reg[r_a]) + " w_b: " + str(w_b), mem=section_arrays)
                ...
                # TODO!!!!!!!!!!!!!!!!!!
                reg[r_a] = pvm_Z_inv_aot(I64(pvm_Z_aot(w_b, 8)) >> I64(U64(v_x) & U64(63)), U8(8))
                if logg: log(logging, local_state, reg, reg1=r_a, reg2=r_b, imm1=v_x,
                                context="w'_a: " + str(reg[r_a]) + " w_b: " + str(w_b), mem=section_arrays)

            elif opcode == op_neg_add_imm_64:
                # 64-bit subtract (v_x - w_b) with wrap
                ...
                reg[r_a] = pvm_Z_inv_aot(I64(pvm_Z_aot(v_x, 8)) >> I64(U64(w_b) & U64(63)), U8(8))
                if logg: log(logging, local_state, reg, reg1=r_a, reg2=r_b, imm1=v_x,
                                context="w'_a: " + str(reg[r_a]) + " w_b: " + str(w_b), mem=section_arrays)

            elif opcode == op_rot_r_64_imm:
                reg[r_a] = rori64_aot(w_b, v_x)
                if logg: log(logging, local_state, reg, reg1=r_a, reg2=r_b, imm1=v_x,
                                context="w'_a: " + str(reg[r_a]) + " w_b: " + str(w_b), mem=section_arrays)

            elif opcode == op_rot_r_64_imm_alt:
                reg[r_a] = rori64_aot(v_x, w_b)
                if logg: log(logging, local_state, reg, reg1=r_a, reg2=r_b, imm1=v_x,
                                context="w'_a: " + str(reg[r_a]) + " w_b: " + str(w_b), mem=section_arrays)

            elif opcode == op_rot_r_32_imm:
                reg[r_a] = pvm_X_aot(rori32_aot(U32(w_b), U32(v_x)), U8(4))
                if logg: log(logging, local_state, reg, reg1=r_a, reg2=r_b, imm1=v_x,
                                context="w'_a: " + str(reg[r_a]) + " w_b: " + str(w_b), mem=section_arrays)

            elif opcode == op_rot_r_32_imm_alt:
                reg[r_a] = pvm_X_aot(rori32_aot(U32(v_x), U32(w_b)), U8(4))
                if logg: log(logging, local_state, reg, reg1=r_a, reg2=r_b, imm1=v_x,
                                context="w'_a: " + str(reg[r_a]) + " w_b: " + str(w_b), mem=section_arrays)
        ...
        # GP-0.6.7-section:A.5.11
            ...
            l_x = min(4, max(0, inst_arg_len[inst_index] - 1))
            v_x = pvm_Z_aot(read_uint_aot(code, pc + 2, l_x), U8(l_x))

            if opcode == op_branch_eq:
                branch_result = branch_aot(pc, v_x, w_a == w_b, pc_to_inst_index)
                if branch_result == I32(-1):
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PANIC, pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_PANIC_INVALID_BRANCH)
                elif w_a == w_b:
                ...
                branch_result = branch_aot(pc, v_x, pvm_Z_aot(w_a, 8) < pvm_Z_aot(w_b, 8), pc_to_inst_index)
                if branch_result == I32(-1):
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PANIC, pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_PANIC_INVALID_BRANCH)
                elif pvm_Z_aot(w_a, 8) < pvm_Z_aot(w_b, 8):
                    skip_len = v_x
                if logg: log(logging, local_state, reg, reg1=r_a, reg2=r_b, off1=v_x,
                                context="skip_len: " + str(skip_len), mem=section_arrays)
        ...
        # GP-0.6.7-section:A.5.12
            ...
            v_x = pvm_X_aot(read_uint_aot(code, pc + 3, l_x), U8(l_x))

            l_y = min(4, max(0, inst_arg_len[inst_index] - l_x - 2))
            v_y = pvm_X_aot(read_uint_aot(code, pc + 3 + l_x, l_y), U8(l_y))

            if opcode == op_load_imm_jump_ind:
                reg[r_a] = v_x
                jump_target = (U64(w_b) + U64(v_y)) & U32_MASK
                djump_result = djump_aot(U32(jump_target), jump_table, pc, pc_to_inst_index)
                if djump_result == I32(-1):
                    skip_len = I64(0)
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_HALT, pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_NONE)
                elif djump_result == I32(-2):
                    return sync_state_and_return_aot(reg, registers_out, state_out, EXIT_PANIC, pc, gas, inst_nr,
                                                     exit_value, skip_len, ERROR_PANIC_INVALID_DJUMP)
        ...
        # GP-0.6.7-section:A.5.13
            ...
            if opcode == op_add_32:
                ...
                reg[r_d] = pvm_X_aot(U32(wa_wb_32), U8(4))
                ...
                reg[r_d] = pvm_X_aot(U32(wa_minus_wb_32), U8(4))
                if logg: log(logging, local_state, reg, reg1=r_d, reg2=r_a, reg3=r_d,
                                context="w'_d: " + str(reg[r_d]), mem=section_arrays)

            elif opcode == op_mul_32:
                prod32 = (U64(w_a) * U64(w_b)) & U32_MASK
                reg[r_d] = pvm_X_aot(U32(prod32), U8(4))
                if logg: log(logging, local_state, reg, reg1=r_d, reg2=r_a, reg3=r_d,
                                context="w'_d: " + str(reg[r_d]), mem=section_arrays)
                ...
                if w_b == 0:
                    ...
                    reg[r_d] = pvm_X_aot(U32(w_a) // U32(w_b), U8(4))
                    if logg: log(logging, local_state, reg, reg1=r_d, reg2=r_a, reg3=r_d,
                                    context="w'_d: " + str(reg[r_d]), mem=section_arrays)

            elif opcode == op_div_s_32:
                # Signed 32-bit division via masked inputs
                a_signed = I32(pvm_Z_aot((U64(w_a) & U32_MASK), 4))
                b_signed = I32(pvm_Z_aot((U64(w_b) & U32_MASK), 4))
                ...
                if b_signed == 0:
                    ...
                    reg[r_d] = pvm_Z_inv_aot(a_signed, U8(8))
                    if logg: log(logging, local_state, reg, reg1=r_d, reg2=r_a, reg3=r_d,
                                    context="w'_d: " + str(reg[r_d]), mem=section_arrays)
                else:
                    reg[r_d] = pvm_Z_inv_aot(pvm_rtz_div_aot(I64(a_signed), I64(b_signed)), U8(8))
                    if logg: log(logging, local_state, reg, reg1=r_d, reg2=r_a, reg3=r_d,
                                    context="w'_d: " + str(reg[r_d]), mem=section_arrays)
                ...
                if wb32 == 0:
                    reg[r_d] = pvm_X_aot(U32(U64(w_a) & U32_MASK), U8(4))
                if logg: log(logging, local_state, reg, reg1=r_d, reg2=r_a, reg3=r_d,
                                    context="w'_d: " + str(reg[r_d]), mem=section_arrays)
                else:
                    wa32 = U64(w_a) & U32_MASK
                    reg[r_d] = pvm_X_aot(U32(wa32 % wb32), U8(4))
                if logg: log(logging, local_state, reg, reg1=r_d, reg2=r_a, reg3=r_d,
                                    context="w'_d: " + str(reg[r_d]), mem=section_arrays)

            elif opcode == op_rem_s_32:
                a_signed = pvm_Z_aot((U64(w_a) & U32_MASK), 4)
                b_signed = pvm_Z_aot((U64(w_b) & U32_MASK), 4)
                ...
                if b_signed == 0:
                    ...
                    reg[r_d] = pvm_Z_inv_aot(pvm_smod_aot(a_signed, b_signed), U8(8))
                    if logg: log(logging, local_state, reg, reg1=r_d, reg2=r_a, reg3=r_d,
                                    context="w'_d: " + str(reg[r_d]), mem=section_arrays)

            elif opcode == op_shlo_l_32:
                sh = U64(w_b) & U64(31)
                reg[r_d] = pvm_X_aot(U32((U64(w_a) << sh) & U32_MASK), U8(4))
                if logg: log(logging, local_state, reg, reg1=r_d, reg2=r_a, reg3=r_d,
                                context="w'_d: " + str(reg[r_d]), mem=section_arrays)

            elif opcode == op_shlo_r_32:
                reg[r_d] = pvm_X_aot(U32(w_a) >> U32(U32(w_b) & U32(31)), U8(4))
                if logg: log(logging, local_state, reg, reg1=r_d, reg2=r_a, reg3=r_d,
                                context="w'_d: " + str(reg[r_d]), mem=section_arrays)

            elif opcode == op_shar_r_32:
                # TODO!!!!!!!!!!!!!!!!!!
                reg[r_d] = pvm_Z_inv_aot(I32(pvm_Z_aot(U32(w_a), 4)) >> I64(U32(w_b) & U32(31)), U8(8))
                if logg: log(logging, local_state, reg, reg1=r_d, reg2=r_a, reg3=r_d,
                                context="w'_d: " + str(reg[r_d]), mem=section_arrays)
                ...
                if w_b == 0:
                    ...
                    if logg: log(logging, local_state, reg, reg1=r_d, reg2=r_a, reg3=r_d,
                                    context="w'_d: " + str(reg[r_d]), mem=section_arrays)
                elif pvm_Z_aot(w_a, 8) == I64(-9223372036854775808) and pvm_Z_aot(w_b, 8) == I64(-1):
                    reg[r_d] = w_a  # Overflow case
                    if logg: log(logging, local_state, reg, reg1=r_d, reg2=r_a, reg3=r_d,
                                    context="w'_d: " + str(reg[r_d]), mem=section_arrays)
                else:
                    reg[r_d] = pvm_Z_inv_aot(pvm_rtz_div_aot(pvm_Z_aot(w_a, 8), pvm_Z_aot(w_b, 8)), U8(8))
                    if logg: log(logging, local_state, reg, reg1=r_d, reg2=r_a, reg3=r_d,
                                    context="w'_d: " + str(reg[r_d]), mem=section_arrays)
                ...
                a_signed = pvm_Z_aot(w_a, 8)
                b_signed = pvm_Z_aot(w_b, 8)
                if b_signed == 0:
                    reg[r_d] = pvm_Z_inv_aot(a_signed, U8(8))
                    if logg: log(logging, local_state, reg, reg1=r_d, reg2=r_a, reg3=r_d,
                                    context="w'_d: " + str(reg[r_d]), mem=section_arrays)
                ...
                reg[r_d] = pvm_Z_inv_aot(I64(pvm_Z_aot(w_a, 8)) >> I64(U64(w_b) & U64(63)), U8(8))
                if logg: log(logging, local_state, reg, reg1=r_d, reg2=r_a, reg3=r_d,
                                context="w'_d: " + str(reg[r_d]), mem=section_arrays)
                ...
                reg[r_d] = U64(1) if pvm_Z_aot(w_a, 8) < pvm_Z_aot(w_b, 8) else U64(0)
                if logg: log(logging, local_state, reg, reg1=r_d, reg2=r_a, reg3=r_d,
                                context="w'_d: " + str(reg[r_d]), mem=section_arrays)
                ...
                reg[r_d] = roli64_aot(w_a, U64(w_b) & U64(63))
                if logg: log(logging, local_state, reg, reg1=r_d, reg2=r_a, reg3=r_d,
                                context="w'_d: " + str(reg[r_d]), mem=section_arrays)

            elif opcode == op_rot_l_32:
                reg[r_d] = pvm_X_aot(roli32_aot(U32(w_a), U32(U32(w_b) & U32(31))), U8(4))
                if logg: log(logging, local_state, reg, reg1=r_d, reg2=r_a, reg3=r_d,
                                context="w'_d: " + str(reg[r_d]), mem=section_arrays)

            elif opcode == op_rot_r_64:
                reg[r_d] = rori64_aot(w_a, U64(w_b) & U64(63))
                if logg: log(logging, local_state, reg, reg1=r_d, reg2=r_a, reg3=r_d,
                                context="w'_d: " + str(reg[r_d]), mem=section_arrays)

            elif opcode == op_rot_r_32:
                reg[r_d] = pvm_X_aot(rori32_aot(U32(w_a), U32(U32(w_b) & U32(31))), U8(4))
                if logg: log(logging, local_state, reg, reg1=r_d, reg2=r_a, reg3=r_d,
                                context="w'_d: " + str(reg[r_d]), mem=section_arrays)
                ...
                reg[r_d] = w_a if pvm_Z_aot(w_a, 8) >= pvm_Z_aot(w_b, 8) else w_b
                if logg: log(logging, local_state, reg, reg1=r_d, reg2=r_a, reg3=r_d,
                                context="w'_d: " + str(reg[r_d]), mem=section_arrays)
...
class PVMInterpreter(PVMInterpreterBase):
    ...
    def _prepare_memory_for_jit(self):
        ...
        if not self.mem_sections:
            empty_acl = Dict.empty(key_type=types.uint32, value_type=types.int32)
