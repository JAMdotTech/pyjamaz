from numba import types
from numba.pycc import CC

#https://numba.readthedocs.io/en/stable/reference/aot-compilation.html
#https://numba.readthedocs.io/en/stable/cuda/caching.html

cc = CC('interpreter_numba_aot_ffi')

# ---- Type aliases ----
U8   = types.uint8
U32  = types.uint32
U64  = types.uint64
I32  = types.int32
I64  = types.int64

U8_A1   = U8[::1]
U32_A1  = U32[::1]
U64_A1  = U64[::1]
I32_A1  = I32[::1]
I64_A1  = I64[::1]

U8_LIST        = types.ListType(U8_A1)                     # List[uint8[:]]
U64_LIST       = types.ListType(U64_A1)                    # List[uint64[:]]
NAMES_LIST_T   = types.ListType(types.unicode_type)        # List[unicode]


from .interpreter_numba_jit import invoke_native_jit


@cc.export(
    'invoke_native',
    I32(                     # return: int32 error_code
        U32, I64, U32, U32,  # pc, gas, inst_nr, skip_len
        U8_A1, U32,          # code, code_size
        I32_A1, I32_A1, I32_A1, I32_A1,  # inst_arg_len, pc->inst, opcode_scheme, jump_table
        U64_A1, U64_A1,      # mem_section_starts/ends
        U8_LIST,             # section_arrays : List[uint8[:]]
        U64_LIST,            # acl_bitmaps
        I32_A1,              # section_access
        U64_A1,              # heap_info
        U64_A1,              # registers_in
        types.boolean,       # logging
        NAMES_LIST_T,        # opcode_names
        U64_A1, I64_A1, I64_A1  # registers_out, state_out, heap_grew_out
    )
)
def invoke_native(
    pc_start, gas_start, inst_start, initial_skip_len,
    code, code_size,
    inst_arg_len_array, pc_to_inst_index, opcode_scheme_array, jump_table_array,
    mem_section_starts, mem_section_ends, section_arrays, acl_bitmaps, section_access,
    heap_info, registers_in,
    logging_enabled, opcode_names,
    registers_out, state_out, heap_grew_out
):
    return invoke_native_jit(
        pc_start, gas_start, inst_start, initial_skip_len,
        code, code_size,
        inst_arg_len_array, pc_to_inst_index, opcode_scheme_array, jump_table_array,
        mem_section_starts, mem_section_ends, section_arrays, acl_bitmaps, section_access,
        heap_info, registers_in,
        logging_enabled, opcode_names,
        registers_out, state_out, heap_grew_out
    )

if __name__ == '__main__':
    cc.compile()
