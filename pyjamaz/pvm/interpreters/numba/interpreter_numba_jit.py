...
@njit(int32(
    ...
    uint64[::1],     # registers_out
    int64[::1],      # state_out (now includes STATE_CURRENT_BLOCK_START at index 7)
...
class PVMInterpreter:
    ...
    def __init__(self, program: PVMProgram, logger=None):
        ...
        self.gas = I64(0)
        self.name = program.name
        # Note: append synthetic trap at end of code (GP-0.7.2-section:A.3 instructions beyond code trigger trap)
        orig_code = np.array(program.code.code, dtype=U8)
        self.code_length = len(orig_code)
        self.code:npt.NDArray[U8] = np.append(orig_code, U8(op_trap))
    ...
    def _link_memory(self, memory):
        ...
        # Also include sections from memory.sections (for test fixtures that use map_section)
    ...
    def invoke(self, pc: int, gas: int):
        ...
        # Advance PC only when resuming (continue execution)
        # Do NOT advance PC for: host_halt, halt, out_of_gas, page_fault, panic
        # These stopping conditions should preserve PC at the instruction that caused them
        if error_code == ERROR_NONE and self.status == ExitReason.resume.value:
