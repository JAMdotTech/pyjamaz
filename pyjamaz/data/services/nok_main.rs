...
extern "C" fn accumulate(start_address: u64, length: u64) -> (u64, u64) {
    ...
    unsafe {
        //call_log(2, None, &format!("LOG_START: {:?} ", work_result_address));

            //DOES NOT WORK: seems to do some magic with copying bytes
        ...
        // This simple version does work
//         for i in 0..(work_result_length as usize) {
//             output_bytes_32[i] = core::ptr::read((work_result_address as *const u8).add(i));
//         }

        output_address = output_bytes_32.as_ptr() as u64;
        output_length = output_bytes_32.len() as u64;
        //call_log(2, None, &format!("LOG_END: {:?} ", work_result_length));
    }
...
extern "C" fn on_transfer(start_address: u64, length: u64) -> (u64, u64) {
    ...
    loop {
        ...
        call_log(2, None, &format!("TRIB on_transfer: timeslot={:?} service_index={:?} sender={:?} receiver={:?} amount={:?} memo={:?} gas_limit={:?}", timeslot, service_index, sender, receiver, amount, memo, gas_limit));
        ...
        call_log(2, None, &format!("TRIB on_transfer gas: got {:?} (recorded at key 4)", gas_result));
