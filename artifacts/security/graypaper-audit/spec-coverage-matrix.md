...
| SAFROLE, assurances, disputes, and report validation | `pyjamaz/validation.py`, `pyjamaz/runtime/pipelines/*`, `pyjamaz/state/components.py`, `pyjamaz/app.py` | `test/test_safrole.py`, `test/test_assurances.py`, `test/test_disputes.py`, `test/test_reports.py`, plus dispute-stage assurance regression coverage in `test/test_assurances.py` | JDT and W3F fixtures already present in `test/fixtures` | `PyjamazApp.state_transition` dispute-to-assurance ordering now matches Gray Paper report clearing, but the wider accumulation path still needs a full `0.8.0` re-audit because code comments and equations remain pinned to `0.7.2`. Owner: core maintainers |
