...
#find "$NUMBA_DIR" -type f \( -name 'interpreter_numba_aot*.so' -o -name 'interpreter_numba_aot*.pyd' -o -name 'interpreter_numba_aot*.dylib' -o -name 'interpreter_numba_aot*.dll' -o -name 'interpreter_numba_aot*.o' -o -name 'interpreter_numba_aot*.c' \) -delete 2>/dev/null || true
#rm -rf "$NUMBA_DIR/__pycache__" 2>/dev/null || true

echo
echo "Compiling interpreter_numba_aot..."
echo "Started at: $(date)"
if python -m pyjamaz.pvm.interpreters.numba.interpreter_numba_aot; then
...
found="$(find "$NUMBA_DIR" -maxdepth 1 -type f -name 'interpreter_numba_aot*' -print)"
if [ -n "$found" ]; then
  ls -lh $found
else
