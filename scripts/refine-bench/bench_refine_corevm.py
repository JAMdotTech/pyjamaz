...
SEED = "0x" + ("00" * 32)
DEFAULT_BUILDER = "./corevm-builder"
DEFAULT_SERVICE = "c36351c2"
LOG_TS_RE = re.compile(r"(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}\.\d{3})")
ANSI_RE = re.compile(r"\x1b\[[0-9;]*m")
REPORT_RE = re.compile(r"(?:Created work-report 0x|work_report_created .* report=0x)([0-9a-fA-F.]+)")
GUARANTEE_RE = re.compile(r"(?:Added guarantee for work-report 0x|guarantee_added report=0x)([0-9a-fA-F.]+)")
WORK_PACKAGE_RECEIVED_RE = re.compile(r"work_package_received wp=0x([0-9a-fA-F.]+)")
REFINE_STARTED_RE = re.compile(r"refine_started wp=0x([0-9a-fA-F.]+)")
REPORT_CREATED_RE = re.compile(r"work_report_created wp=0x([0-9a-fA-F.]+) report=0x([0-9a-fA-F.]+)")
REPORTED_STATUS_SENT_RE = re.compile(r"reported_status_sent wp=0x([0-9a-fA-F.]+) status=([A-Za-z]+)")
...
def parse_node_log_refines(node_log_path: Path) -> list[dict[str, Any]]:
    ...
    for raw_line in node_log_path.read_text(encoding="utf-8", errors="ignore").splitlines():
        ...
        if "Processing work-package" in line or "refine_started" in line:
            started_at = timestamp
        elif started_at is not None and ("Created work-report" in line or "work_report_created" in line):
...
def summarize_cycle_events(node_log_path: Path) -> dict[str, Any]:
    ...
    for raw_line in node_log_path.read_text(encoding="utf-8", errors="ignore").splitlines():
        ...
        if match := WORK_PACKAGE_RECEIVED_RE.search(line):
            events.append({"kind": "received", "timestamp": timestamp, "wp": match.group(1)})
        if match := REFINE_STARTED_RE.search(line):
            events.append({"kind": "refine_started", "timestamp": timestamp, "wp": match.group(1)})
        if match := REPORT_CREATED_RE.search(line):
            events.append({"kind": "report_created", "timestamp": timestamp, "wp": match.group(1), "report": match.group(2)})
        if match := REPORTED_STATUS_SENT_RE.search(line):
            events.append({"kind": "reported_sent", "timestamp": timestamp, "wp": match.group(1), "status": match.group(2)})

    report_to_status = []
    received_to_refine = []
    reported_to_next_refine = []
        ...
        if event["kind"] == "report_created":
            status_event = next(
                (
                    ...
                    if candidate["kind"] == "reported_sent" and
                    candidate["wp"] == event["wp"] and
                    candidate["timestamp"] >= event["timestamp"]
                ),
                None,
            )
            if status_event is not None:
                report_to_status.append((status_event["timestamp"] - event["timestamp"]).total_seconds())
        elif event["kind"] == "received":
            refine_event = next(
                (
                    candidate for candidate in events
                    if candidate["kind"] == "refine_started" and
                    candidate["wp"] == event["wp"] and
                    candidate["timestamp"] >= event["timestamp"]
                ),
                None,
            )
            if refine_event is not None:
                received_to_refine.append((refine_event["timestamp"] - event["timestamp"]).total_seconds())
        elif event["kind"] == "reported_sent":
            refine_event = next(
                (
                    candidate for candidate in events
                    if candidate["kind"] == "refine_started" and
            ...
            if refine_event is not None:
                reported_to_next_refine.append((refine_event["timestamp"] - event["timestamp"]).total_seconds())

    return {
        "event_count": len(events),
        "work_packages_received": sum(1 for event in events if event["kind"] == "received"),
        "refine_started_count": sum(1 for event in events if event["kind"] == "refine_started"),
        "reported_sent_count": sum(1 for event in events if event["kind"] == "reported_sent"),
        "report_created_to_reported_sent": report_to_status,
        "reported_sent_to_next_refine_started": reported_to_next_refine,
        "work_package_received_to_refine_started": received_to_refine,
        "median_report_created_to_reported_sent": statistics.median(report_to_status) if report_to_status else None,
        "median_reported_sent_to_next_refine_started": statistics.median(reported_to_next_refine) if reported_to_next_refine else None,
        "median_work_package_received_to_refine_started": statistics.median(received_to_refine) if received_to_refine else None,
    }
...
def run_one(args: argparse.Namespace, repo: Path, out_dir: Path, run_index: int, measured: bool) -> dict[str, Any]:
    ...
    try:
        ...
        cycle = summarize_cycle_events(node_log_path)
    finally:
        terminate(node)
...
def main() -> None:
    ...
    parser.add_argument("--max-next-submit-to-refine-start", type=float, default=None)
    ...
    report_created_to_reported_sent = [
        value
        for s in summaries
        for value in s.get("cycle", {}).get("report_created_to_reported_sent", [])
    ]
    reported_sent_to_next_refine_started = [
        value
        for s in summaries
        for value in s.get("cycle", {}).get("reported_sent_to_next_refine_started", [])
    ]
    work_package_received_to_refine_started = [
        value
        for s in summaries
        for value in s.get("cycle", {}).get("work_package_received_to_refine_started", [])
    ]
    stage_summary = {
        "runs": summaries,
        ...
        "cycle": {
            "report_created_to_reported_sent": report_created_to_reported_sent,
            "reported_sent_to_next_refine_started": reported_sent_to_next_refine_started,
            "work_package_received_to_refine_started": work_package_received_to_refine_started,
            "median_report_created_to_reported_sent": (
                statistics.median(report_created_to_reported_sent)
                if report_created_to_reported_sent else None
            ),
            "median_reported_sent_to_next_refine_started": (
                statistics.median(reported_sent_to_next_refine_started)
                if reported_sent_to_next_refine_started else None
            ),
            "median_work_package_received_to_refine_started": (
                statistics.median(work_package_received_to_refine_started)
                if work_package_received_to_refine_started else None
            ),
        },
    }
    speedup = None
    ...
    if args.require_reported_status_sent:
        missing = [s["run_index"] for s in summaries if not s.get("cycle", {}).get("reported_sent_count")]
        if missing:
            raise SystemExit(f"Reported status send was not observed for measured runs: {missing}")
    ...
    if args.max_reported_to_next_refine_start is not None:
        cycle_values = stage_summary["cycle"]["reported_sent_to_next_refine_started"]
        if not cycle_values:
            raise SystemExit("No reported_sent -> next refine_started cycle observations were recorded")
        too_slow = [
            value for value in cycle_values
            if value > args.max_reported_to_next_refine_start
        ]
        if too_slow:
            raise SystemExit(
                f"Cycle latency limit failed: {len(too_slow)} cycle(s) exceeded "
                f"{args.max_reported_to_next_refine_start:.3f}s; max={max(too_slow):.6f}s"
            )
    if args.max_next_submit_to_refine_start is not None:
        cycle_values = stage_summary["cycle"]["work_package_received_to_refine_started"]
        if not cycle_values:
            raise SystemExit("No work_package_received -> refine_started cycle observations were recorded")
        too_slow = [
            value for value in cycle_values
            if value > args.max_next_submit_to_refine_start
        ]
        if too_slow:
            raise SystemExit(
                f"Submit-to-refine latency limit failed: {len(too_slow)} cycle(s) exceeded "
                f"{args.max_next_submit_to_refine_start:.3f}s; max={max(too_slow):.6f}s"
            )
